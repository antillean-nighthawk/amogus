        ...
        for i in list(messages.keys())[:cwnd]:
            ...
            try:    
                ...
                if cwnd >= ssthresh: # slow start => aimd
                    slow_start = False
                elif ack_count[ack_id] >= 3: # fast retransmit => slow start
                    # treat 3 dupes as packet loss (same as timeout)
                    slow_start = True
                    ssthresh = cwnd / 2
                    cwnd = 0.5 # will become 1 in if statment below
                ...
                # if packet loss, switch back to slow start
                slow_start = True
                ssthresh = cwnd / 2
                cwnd = 1
